...
class AudioRecordingService : Service() {
    ...
    override fun onCreate() {
        ...
        //Log.d(TAG, "Recording buffer size: $recordingBufferSize")
    ...
    private fun record() {
        ...
        while (isRecording) {
            ...
            //Log.v(TAG, "Recorded buffer: ${recordingBuffer.size}")
            //Log.v(TAG, "Recorded buffer: ${recordingBuffer.toList()}")

            val energy = calculateEnergy(recordingBuffer)
            Log.d(TAG, "Recording energy: $energy")
    ...
    private fun computeBuffer(audioBuffer: DoubleArray) {
        //var doubleBuffer = audioBuffer.map { it.toDouble() }.toDoubleArray()
    ...
    private fun loadAndPredict(mfccs: FloatArray) {
        ...
        //Log.d(TAG, "Input shape: ${Arrays.toString(imageShape)}")
        //Log.d(TAG, "Output shape: ${Arrays.toString(probabilityShape)}")
        ...
        if (axisLabels != null) {
            ...
            for (label in labels) {
                ...
                //Log.d(TAG, "Label: ${label.key}, Value: ${label.value}")
            }
    ...
    private fun calculateEnergy(buffer: DoubleArray): Double {
        var energy = 0.0
